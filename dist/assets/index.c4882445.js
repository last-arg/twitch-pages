import{m as g}from"./vendor.e906ad84.js";const E=function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))r(t);new MutationObserver(t=>{for(const o of t)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&r(n)}).observe(document,{childList:!0,subtree:!0});function e(t){const o={};return t.integrity&&(o.integrity=t.integrity),t.referrerpolicy&&(o.referrerPolicy=t.referrerpolicy),t.crossorigin==="use-credentials"?o.credentials="include":t.crossorigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function r(t){if(t.ep)return;t.ep=!0;const o=e(t);fetch(t.href,o)}};E();const x=100,U="7v5r973dmjp0nd1g43b8hcocj2airz",N=10,d="/twitch-pages",$={"top-games":{url:d,html:d+"/partials/top-games.html"},category:{url:d+"/directory/game/:name",html:d+"/partials/category.html"},"user-videos":{url:d+"/:user/videos",html:d+"/partials/user-videos.html"},"not-found":{url:d+"/not-found",html:d+"/partials/not-found.html"}};const G=l=>{if(l===d)return $["top-games"];let s="not-found";const e=l.split("/").filter(r=>r.length>0);for(const r in $){const o=$[r].url.split("/").filter(c=>c.length>0);if(o.length!==e.length||o.length===0)continue;let n=!0;for(let c=0;c<o.length;c+=1){const f=o[c];if(f[0]!==":"&&f!==e[c]){n=!1;break}}if(n){s=r;break}}return $[s]};function j(l){const s={Host:"api.twitch.tv",Authorization:`Bearer ${l}`,"Client-id":U,Accept:"application/vnd.twitchtv.v5+json"},e=async r=>{if(r.length===0)return[];const t=`https://api.twitch.tv/helix/users?id=${r.join("&id=")}`;return(await(await fetch(t,{method:"GET",headers:s})).json()).data};document.addEventListener("alpine:initializing",()=>{const r=document.querySelector("#search-feedback");g.data("sidebar",()=>({state:"closed",loading:!1,searchValue:"",searchResults:[],init(){let t=0,o=document.activeElement;g.effect(()=>{document.activeElement?.tagName==="BUTTON"&&(this.state==="closed"?o.focus():o=document.activeElement)}),g.effect(()=>{clearTimeout(t);const n=this.searchValue.trim();n.length>0&&(this.loading=!0,t=setTimeout(async()=>{r.textContent="Searching games",this.searchResults=await this.fetchSearch(n),this.loading=!1,this.searchResults.length===1?r.textContent="Found one game":this.searchResults.length>1?r.textContent=`Found ${this.searchResults.length} games`:r.textContent="Found no games"},400))})},async fetchSearch(t){const o=t.trim();if(o.length===0)return[];const n=`https://api.twitch.tv/helix/search/categories?first=${N}&query=${o}`;return(await(await fetch(n,{method:"GET",headers:s})).json()).data??[]},toggleSidebar(t){if(this.state===t){this.state="closed";return}this.state=t},getImageSrc(t,o,n){return L(t,o,n)},sidebarToSvgIconId(t){switch(t){case"games":return"game-controller";case"streams":return"people";case"search":return"looking-class"}return""}}))}),document.addEventListener("alpine:init",function(){const r={data:JSON.parse(localStorage.getItem("games")??"[]"),ids:[],init(){g.effect(()=>{this.ids=this.data.map(({id:a})=>a),localStorage.setItem("games",JSON.stringify(this.data))})},hasId(a){return this.ids.includes(a)},toggle(a,i){return this.hasId(a)?(this.remove(a),!1):(this.add(a,i),!0)},add(a,i){let h=0;for(const{name:u}of this.data){if(i<u)break;h+=1}this.data.splice(h,0,{id:a,name:i})},remove(a){const i=this.ids.indexOf(a);i!==-1&&this.data.splice(i,1)}},t=async a=>{if(a.length===0)return[];const i=`https://api.twitch.tv/helix/streams?user_id=${a.join("&user_id=")}&first=${x}`;return(await(await fetch(i,{method:"GET",headers:s})).json()).data},o="streams",n=`${o}.live`,c=`${n}.last_check`,f=3e5,p={data:JSON.parse(localStorage.getItem(o)??"[]"),ids:[],live:JSON.parse(localStorage.getItem("streams.live")??"{}"),liveLastCheck:parseInt(JSON.parse(localStorage.getItem(c)??Date.now().toString()),10),init(){g.effect(()=>{this.ids=this.data.map(({user_id:i})=>i),localStorage.setItem(o,JSON.stringify(this.data))}),g.effect(()=>{localStorage.setItem(n,JSON.stringify(this.live))}),g.effect(()=>{localStorage.setItem(c,JSON.stringify(this.liveLastCheck))});const a=()=>{this.updateUserLiveness(this.ids),setTimeout(a,f)};a()},hasId(a){return this.ids.includes(a)},toggle(a,i,h){return this.hasId(a)?(this.remove(a),!1):(this.add(a,i,h),this.addLiveUser(a),!0)},add(a,i,h){let u=0;for(const{user_login:v}of this.data){if(i<v)break;u+=1}this.data.splice(u,0,{user_id:a,user_login:i,user_name:h})},remove(a){const i=this.ids.indexOf(a);i!==-1&&this.data.splice(i,1)},isLive(a){return this.live[a]!==void 0},async addLiveUser(a){if(this.live[a]===void 0){const i=(await t([a]))[0];i&&(this.live[i.user_id]=i.game_name)}},async updateUserLiveness(a){if(a.length===0)return;const i=Date.now(),h=Math.ceil(a.length/x);let u={};for(let v=0;v<h;v+=1){const w=v*x,k=w+x,y=await t(a.slice(w,k));for(const{user_id:C,game_name:T}of y)u[C]=T}this.live=u,this.liveLastCheck=i}},m={data:JSON.parse(localStorage.getItem("profile_images")??"{}"),lastUpdate:parseInt(JSON.parse(localStorage.getItem("profile_images_last_update")??Date.now().toString()),10),init(){const a=Object.keys(this.data),i=p.data.map(({user_id:h})=>h).filter(h=>!a.includes(h));i.length>0&&this.fetchProfileImages(i),g.effect(()=>{localStorage.setItem("profile_images",JSON.stringify(this.data))}),g.effect(()=>{localStorage.setItem("profile_images_last_update",JSON.stringify(this.lastUpdate))}),window.addEventListener("unload",()=>this.clean())},hasId(a){return this.data[a]!==void 0},imgUrl(a){const i=this.data[a];return i?(i.last_access=Date.now(),i.url):""},setImage(a,i){this.data[a]={url:i,last_access:Date.now()}},async fetchProfileImages(a){if(a.length===0)return;const i=Date.now(),h=Math.ceil(a.length/x);for(let u=0;u<h;u+=1){const v=u*x,w=v+x,k=await e(a.slice(v,w));let y={};for(let{id:C,profile_image_url:T}of k)y[C]={url:T,last_access:i};this.data={...this.data,...y}}},clean(a=[]){const i=864e5,h=Date.now();if(h-this.lastUpdate>=i){const v=Object.keys(this.data).filter(w=>!a.includes(w));for(let w of v)h-this.data[w].last_access>i&&delete this.data[w]}this.lastUpdate=h}};g.store("global",{urlRoot:d}),g.store("games",r),g.store("streams",p),g.store("profile_images",m)})}const A=()=>{const l=document.querySelectorAll(".scrollbox");for(const s of l){const e=s.closest(".scroll-container"),r=s.querySelector("ul");let t=!1;const o=n=>{if(!t){const c=n.target;window.requestAnimationFrame(function(){c.scrollTop>0?e.classList.add("has-top-shadow"):e.classList.remove("has-top-shadow"),c.scrollTop+c.offsetHeight<r.offsetHeight?e.classList.add("has-bottom-shadow"):e.classList.remove("has-bottom-shadow"),t=!1}),t=!0}};s.addEventListener("scroll",o)}},L=(l,s,e)=>`https://static-cdn.jtvnw.net/ttv-boxart/${l}-${s}x${e}.jpg`;function O(l,s,e){return L(l,s,e)}function D(l,s,e){return l.replace("{width}",s.toString()).replace("{height}",e.toString())}const b=104,S=144,_=440,I=248;window.gameClicked=null;window.userClicked=null;const R=l=>{let s="";for(const e of l)s+=`
  ...
  `},H=l=>{let s="";for(const e of l){const r=`${d}/${e.user_login}/videos`;s+=`
      <li class="fade-in">
        <div>
          ...
          >
            <div class="relative">
              <img class="rounded" src="${D(e.thumbnail_url,_,I)}" alt="" width="${_}" height="${I}" />
          ...
          <div class="flex bg-white rounded px-1 py-1.5 border-t-2 border-trueGray-50">
            ...
            <a aria-hidden="true" href="${r}"
              hx-push-url="${r}" hx-get="${$["user-videos"].html}" hx-target="#main"
   ...
   `},J={archive:"bg-lime-200",upload:"bg-sky-200",highlight:"bg-amber-200"},M={archive:"Archive",upload:"Upload",highlight:"Highlight"},V={archive:"video-camera",upload:"video-upload",highlight:"video-reel"},P=l=>{const s=h=>{const u=Math.floor(h);return h-u>.5?Math.ceil(h):u},r=(Date.now()-l.getTime())/1e3/60,t=r/60,o=t/24,n=s(r),c=s(t),f=s(o),p=s(o/7),m=s(o/30),a=s(o/365);let i="1 minute ago";return a>0&&m>11?i=a===1?"1 year ago":`${a} years ago`:m>0&&p>4?i=m===1?"1 month ago":`${m} months ago`:p>0&&f>6?i=p===1?"1 week ago":`${p} weeks ago`:f>0&&c>23?i=f===1?"Yesterday":`${f} days ago`:c>0&&n>59?i=c===1?"1 hour ago":`${c} hours ago`:n>1&&(i=`${n} minutes ago`),i};function z(l,s,e){return l.replace("%{width}",s.toString()).replace("%{height}",e.toString())}const B=l=>{let s="";for(const e of l){const r=new Date(e.published_at);s+=`
      <li class="fade-in">
        <a class="block group" href="${e.url}" title="${e.title}">
          <div class="relative">
          <img class="w-full rounded-sm" src="${z(e.thumbnail_url,_,I)}" alt="" width="${_}" height="${I}" />
            <span class="opacity-90 absolute top-0 left-0 mt-1.5 ml-1.5 px-0.5 rounded-sm ${J[e.type]}"
              title="${M[e.type]}"
            >
    ...
    `}return s},X=async l=>{htmx.defineExtension("twitch-api",{onEvent:function(s,e){if(s==="htmx:configRequest"){e.detail.headers["HX-Current-URL"]=null,e.detail.headers["HX-Request"]=null,e.detail.headers["HX-Target"]=null,e.detail.headers["HX-Trigger"]=null,e.detail.headers.Authorization=`Bearer ${l}`,e.detail.headers["Client-id"]=U,e.detail.headers.Accept="application/vnd.twitchtv.v5+json";const r=new URL(e.detail.path);if(r.pathname==="/helix/games"){let t="";if(window.gameClicked)t=window.gameClicked;else{const o=location.pathname.split("/");t=decodeURIComponent(o[o.length-1])}e.detail.parameters.name=t,window.gameClicked=null}else if(r.pathname==="/helix/users"){let t="";if(window.userClicked)t=window.userClicked;else{const o=location.pathname.split("/");t=decodeURIComponent(o[o.length-2])}e.detail.parameters.login=t}}},transformResponse:function(s,e,r){const t=JSON.parse(s);let o="";const n=new URL(e.responseURL);if(n.pathname==="/helix/games")if(e.status===200)document.querySelector(".category-param[name='game_id']")?.setAttribute("value",t.data[0].id),htmx.trigger("#load-more-streams","click",{}),o=q(t);else{const c=location.pathname.split("/");o=`
          ...
          `}else if(n.pathname==="/helix/videos")if(t.data.length>0){o=B(t.data),t.pagination!==void 0&&t.pagination.cursor&&document.querySelector(".req-param[name='after']")?.setAttribute("value",t.pagination.cursor);const c=document.querySelector("#highlights-count"),f=document.querySelector("#uploads-count"),p=document.querySelector("#archives-count"),m={archives:parseInt(p.textContent,10)??0,uploads:parseInt(f.textContent,10)??0,highlights:parseInt(c.textContent,10)??0};for(const a of t.data)a.type==="archive"?m.archives+=1:a.type==="upload"?m.uploads+=1:a.type==="highlight"&&(m.highlights+=1);c.textContent=m.highlights.toString(),f.textContent=m.uploads.toString(),p.textContent=m.archives.toString()}else o='<div id="feedback" hx-swap-oob="true">Found no videos</div>';return o}}),htmx.ajax("GET",G(location.pathname).html,"#main")},W=async()=>{let l=localStorage.getItem("twitch_token");if(window.location.hash)for(const s of window.location.hash.slice(1).split("&")){const[e,r]=s.split("=");if(e==="access_token"){l=r,localStorage.setItem("twitch_token",r);break}}if(l)j(l),g.start(),X(l),A();else{const s=document.querySelector(".js-twitch-login");s.parentElement?.classList.remove("hidden"),s.href=`https://id.twitch.tv/oauth2/authorize?client_id=${U}&redirect_uri=${window.location.origin+window.location.pathname}&response_type=token&scope=`}};W();
